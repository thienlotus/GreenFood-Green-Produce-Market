...
| 2 | **Lê Thiều Hưng**<br>([@0912lethieuhung-hub](https://github.com/0912lethieuhung-hub)) | **Developer & QA/QC Lead** | **Auth & User Service** | [`auth-service`](https://github.com/thienlotus/GreenFood-Green-Produce-Market/tree/auth-service) | **Đã làm:**<br>- Chức năng Đăng ký, Đăng nhập, Quản lý hồ sơ tài khoản (Khách lẻ & Khách sỉ).<br>- Phát triển bộ công cụ chọn địa chỉ hành chính Việt Nam đa cấp (Cascading Dropdown: 63 Tỉnh/Thành - Quận/Huyện - Phường/Xã + Địa chỉ chi tiết), thay thế ô nhập text thủ công, có parser và format chuẩn hóa.<br>- Sửa lỗi bảo mật Sổ địa chỉ (Address Book Data Isolation): Khắc phục triệt để lỗ hổng rò rỉ dữ liệu (tài khoản này nhìn thấy địa chỉ tài khoản khác), phân vùng lưu trữ theo `userId`, xóa sạch dữ liệu nhạy cảm khi đăng xuất (`Zero Data Leakage`).<br>**Công việc bổ sung:**<br>- Thiết kế migration bổ sung cột `address` vào bảng `users` (Laravel 11), tích hợp cập nhật địa chỉ vào `UserService` và `AuthController`.<br>- Xây dựng bảo mật xác thực Session/Token, mã hóa mật khẩu an toàn (`Hash::make`), kiểm soát lỗi đăng nhập.<br>- Phân quyền người dùng (Customer, B2B, Farmer, Admin).<br>- Chủ trì mảng QA/QC: Soạn thảo bộ test cases kiểm thử Đăng ký (`Test_Cases_GreenFood_Register.xlsx`).<br>- Quản lý Bug Lifecycle trên Jira Software Cloud (`GREEN-2`, `GREEN-3`), theo dõi và nghiệm thu đóng/mở lỗi. |
...
├── auth-service           -> origin/auth-service (Lê Thiều Hưng)
...
### 1. Auth & User Service (Phụ trách: Lê Thiều Hưng)
...
- **Lê Thiều Hưng** — Developer & QA/QC Lead ([@0912lethieuhung-hub](https://github.com/0912lethieuhung-hub))
